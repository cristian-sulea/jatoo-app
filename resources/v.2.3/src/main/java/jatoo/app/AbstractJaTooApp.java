...
public abstract class AbstractJaTooApp extends SpringBean implements JaTooApp {
	...
	private boolean isTrayIconClicked = false;
		...
		if (SystemTray.isSupported()) {
			...
			trayIcon.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (!isTrayIconClicked) {
						if (trayIconActionListener != null) {
							trayIconActionListener.actionPerformed(e);
						}
					}

					isTrayIconClicked = false;
				}
			});
	...
	@Override
	public ActionListener getTrayIconActionListener() {
		return trayIconActionListener;
	}
	...
	@Override
	public void showTrayMessage(String caption, String message) {
		if (trayIcon != null) {
			isTrayIconClicked = false;
			trayIcon.displayMessage(caption, message, MessageType.NONE);
		} else {
			getLogger().info("system tray not supported: " + caption + ": " + message);
		}
	}

	@Override
	public void showTrayErrorMessage(String caption, String message) {
		if (trayIcon != null) {
			isTrayIconClicked = false;
			trayIcon.displayMessage(caption, message, MessageType.ERROR);
		} else {
			getLogger().error("system tray not supported: " + caption + ": " + message);
		}
	}

	@Override
	public void showTrayWarningMessage(String caption, String message) {
		if (trayIcon != null) {
			isTrayIconClicked = false;
			trayIcon.displayMessage(caption, message, MessageType.WARNING);
		} else {
			getLogger().warn("system tray not supported: " + caption + ": " + message);
		}
	}

	@Override
	public void showTrayInformationMessage(String caption, String message) {
		if (trayIcon != null) {
			isTrayIconClicked = false;
